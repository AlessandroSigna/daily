...
<html>
	...
	<body>
		...
		<div id="resultPage">
			...
			<?php  echo '<h1>Il tuo carrello</h1><br><br><br><br>'; 
...
{	
 ...
 echo' <div id="container"><div class="descList"><a>Prezzo totale: EUR '.$prezzoTotale.' </a><br><br>';
 	...
 	$queryz=$mysqli->query("SELECT * FROM carrello WHERE ref_cliente='$_SESSION[mailUtente]'");
	if (mysqli_num_rows($queryz)!=0)
	{
		echo '
		<form action="/Daily/payment.php" id="proceed"><input type="submit" value="Procedi all\'acquisto"></form></div></div>
		';
	}
	else
	{
		echo '
		<form action="/Daily/payment.php" id="proceed"><input type="submit" value="Il carrello è vuoto" disabled></form></div></div>
		';		
	}
?>
			<br><br>
